...
import static com.paperflywings.user.paperflyv0.PickupAutoAssignManager.AutoAssignMyPickuplist.CREATED_AT;
import static com.paperflywings.user.paperflyv0.PickupAutoAssignManager.AutoAssignMyPickuplist.MERCHANT_ID;
import static com.paperflywings.user.paperflyv0.PickupAutoAssignManager.AutoAssignMyPickuplist.MERCHANT_NAME;
import static com.paperflywings.user.paperflyv0.PickupAutoAssignManager.AutoAssignMyPickuplist.SUB_MERCHANT_NAME;
...
//    public static final String UPDATE_SCAN_AND_PICKED= "http://paperflybd.com/update_tbl_auto_insertassign_pickup.php";
    ...
    protected void onCreate(Bundle savedInstanceState) {
      ...
      if( sub_merchant_name.equals("") || sub_merchant_name == null) {
            merchant_name_textview.setText("Scan started for: " +merchant_name);
      } else {
            merchant_name_textview.setText("Scan started for: " +sub_merchant_name);
        }
...
//            final String updated_at = currentDateTimeString;
            ...
            if ( lastText.trim().length() != 12 ) {
                ...
                Toast.makeText(AutoScanningScreen.this, "garbage", Toast.LENGTH_LONG).show();
...
//                    getData(username);
//                    sentOnChannel1();
                    Intent intent = new Intent(view.getContext(), AutoAssignMyPickuplist.class);
...
//                                Toast.makeText(ScanningScreen.this, "Barcode Number Added" ,  Toast.LENGTH_LONG).show();
                                Toast toast= Toast.makeText(AutoScanningScreen.this,
                    ...
                    public void onResponse(String response) {
                        try {
                            ...
                            if (!obj.getBoolean("error")) {
                                ...
                                //storing the name to sqlite with status synced
//                                db.add(merchant_id, lastText, state, updated_by, updated_at,);
                                db.update_row_auto_assign(merchantCode,pickMerName, executiveName,scanCount,pickedQty,updatedBy,updatedAt,pickedStatus, NAME_SYNCED_WITH_SERVER);
                            } else {
                                //if there is some error
                                //saving the name to sqlite with status unsynced
                                db.update_row_auto_assign(merchantCode,pickMerName, executiveName,scanCount,pickedQty,updatedBy,updatedAt,pickedStatus, NAME_NOT_SYNCED_WITH_SERVER);
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
