...
public class DeliverySupReturnListAdapter  extends RecyclerView.Adapter<DeliverySupReturnListAdapter.ViewHolder>implements Filterable {
    private List<DeliverySupReturnListModel> listfull;
    private List<DeliverySupReturnListModel> list;

    private int currentPosition = -1;
    private Context context;

    public DeliverySupReturnListAdapter(List<DeliverySupReturnListModel>list, Context context){
        this.list = list;
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
       ...
       /* public TextView item_pickdropby;
        ...
        public TextView item_ret_schedule;
...
//            item_pickMerchantName=itemView.findViewById(R.id.pick_m_name);
        /*    item_orderdate=itemView.findViewById(R.id.sup_order_date);
            ...
            item_ret_schedule = itemView.findViewById(R.id.sup_ret_time);
            item_ret_by = itemView.findViewById(R.id.sup_ret_by);
            item_ret_reason = itemView.findViewById(R.id.sup_returnReason);
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View v  = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.delivery_sup_return_list,viewGroup,false);
    ...
    /*    viewHolder.item_orderdate.setText("Order Date: "+list.get(i).getOrderDate());
        ...
        viewHolder.item_ret_schedule.setText("Return Time: "+list.get(i).getRetTime());
        viewHolder.item_ret_reason.setText(list.get(i).getRetReason());
        viewHolder.item_ret_by.setText("Return Request By: "+list.get(i).getRetBy());
        if(DeliveryTime<0) {
    ...
    private Filter NamesFilter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            List<DeliverySupReturnListModel>filteredList = new ArrayList<>();
            if (constraint == null || constraint.length() == 0){
                filteredList.addAll(listfull);
            }else {
                String filterPattern = constraint.toString().toLowerCase().trim();
                for(DeliverySupReturnListModel item: listfull){
